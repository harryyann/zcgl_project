...
from apps.users.models import UserOperateLog, UserProfile
...
class ServerAddView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        owner = UserProfile.objects.get(pk=request.POST.get('owner'))
        print(type(owner))
        ...
        if server_form.is_valid():
            ...
            # 该记录添加到表中
            server= Server(zctype=zctype, ipaddress=ipaddress,
                                   description=description, brand=brand, zcmodel=zcmodel, zcnumber=zcnumber,
                                   zcpz=zcpz, owner=owner, undernet=undernet, guartime=guartime, comment=comment)
            server.save()

            # 将操作记录添加到日志中
            new_log = UserOperateLog(username=request.user.username, scope=zctype, type='增加',
                                     content=server.pk)
            new_log.save()
...
class ServerDetailView(LoginRequiredMixin, View):
    def get(self, request, server_id):
        ...
        users = UserProfile.objects.filter(is_superuser=0, is_staff='1')
        server_types = ServerType.objects.all()
        server_hiss = Server.objects.filter(id=server_id).order_by('-modify_time')
...
class ServerModifyView(LoginRequiredMixin, View):
    def post(self, request):
        ...
        if server_form.is_valid():
            ...
            # 该记录添加到表中
            server_his = Server(id=server.id, zctype=server.zctype, ipaddress=server.ipaddress,
                                   description=server.description, brand=server.brand, zcmodel=server.zcmodel,
                                   zcnumber=server.zcnumber, zcpz=server.zcpz, owner=server.owner,
